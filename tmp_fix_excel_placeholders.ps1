...
function Set-CellInline([string]$xml,[string]$cellRef,[string]$text){
  ...
  $cellPattern="<c\s+r=\"$cellRef\"[^>]*>[\s\S]*?</c>"
  ...
  if([regex]::IsMatch($xml,$rowPattern)){
    return [regex]::Replace($xml,$rowPattern,[System.Text.RegularExpressions.MatchEvaluator]{ param($m) $m.Groups[1].Value + $m.Groups[2].Value + $replacement + $m.Groups[3].Value },1)
  }
  return $xml
}
...
function Remove-Merges([string]$xml,[int]$startRow,[int]$endRow){
  $shouldRemove={
    param($ref)
    if($ref -notmatch '^([A-Z]+)(\d+):([A-Z]+)(\d+)$'){ return $false }
    $c1=$Matches[1]; $r1=[int]$Matches[2]; $c2=$Matches[3]; $r2=[int]$Matches[4]
    if($r1 -ne $r2){ return $false }
    if($r1 -lt $startRow -or $r1 -gt $endRow){ return $false }
    return (($c1 -eq 'D' -and $c2 -eq 'H') -or ($c1 -eq 'E' -and $c2 -eq 'I') -or ($c1 -eq 'D' -and $c2 -eq 'I'))
  }

  $xml=[regex]::Replace($xml,'<mergeCell\s+ref="([^"]+)"\s*/>',[System.Text.RegularExpressions.MatchEvaluator]{
    param($m)
    $ref=$m.Groups[1].Value
    if(& $shouldRemove $ref){ '' } else { $m.Value }
  })

  $xml=[regex]::Replace($xml,'<mergeCells\s+count="(\d+)"\s*>([\s\S]*?)</mergeCells>',[System.Text.RegularExpressions.MatchEvaluator]{
    param($m)
    $inner=$m.Groups[2].Value
    $list=[regex]::Matches($inner,'<mergeCell\s+ref="[^"]+"\s*/>') | ForEach-Object { $_.Value }
