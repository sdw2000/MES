...
function Set-CellInline([string]$xml,[string]$cellRef,[string]$text){
  ...
  if([regex]::IsMatch($xml,$pattern1)){
    return [regex]::Replace($xml,$pattern1,[System.Text.RegularExpressions.MatchEvaluator]{ param($m) $replacement },1)
  }
  $pattern2='<c\s+r="' + [regex]::Escape($cellRef) + '"[^>]*/>'
  if([regex]::IsMatch($xml,$pattern2)){
    return [regex]::Replace($xml,$pattern2,[System.Text.RegularExpressions.MatchEvaluator]{ param($m) $replacement },1)
  }
...
function Patch-Template([string]$file,[int]$baseRow,[string[]]$resultCols,[string[]]$statusCols){
  Write-Host ('Patching template: ' + $file)
  ...
  try{
    ...
    for($r=0;$r -lt $items.Count;$r++){
      $row=$baseRow + $r
      for($i=0;$i -lt 5;$i++){
        $ref=$resultCols[$i] + $row
        $ph='${' + $items[$r] + '_actual' + ($i+1) + '}'
        $xml=Set-CellInline $xml $ref $ph
      }

      foreach($sc in $statusCols){
        $sref=$sc + $row
        $sph='${' + $items[$r] + '_status}'
        $xml=Set-CellInline $xml $sref $sph
      }
    }
...
foreach($f in $rp01Files){ if(Test-Path $f){ Patch-Template $f 16 @('E','F','G','H','I') @('J','K') } }

$normalFiles=@(
  'E:\vue\ERP\public\downloads\normal_1.placeholders.xlsx',
  'E:\vue\ERP\public\downloads\normal_1.xlsx'
)
foreach($f in $normalFiles){ if(Test-Path $f){ Patch-Template $f 15 @('D','E','F','G','H') @('J') } }
