-- =====================================================
-- 生产管理系统 - 排程相关表
-- 第二阶段：排程主表、各工序计划表、订单状态扩展
-- =====================================================

-- 1. 修改订单明细表，增加排程和交付跟踪字段
ALTER TABLE `sales_order_items` 
ADD COLUMN `scheduled_qty` INT DEFAULT 0 COMMENT '已排程数量(卷)' AFTER `rolls`,
ADD COLUMN `produced_qty` INT DEFAULT 0 COMMENT '已生产数量(卷)' AFTER `scheduled_qty`,
ADD COLUMN `delivered_qty` INT DEFAULT 0 COMMENT '已出货数量(卷)' AFTER `produced_qty`,
ADD COLUMN `remaining_qty` INT DEFAULT 0 COMMENT '未交货数量(卷)' AFTER `delivered_qty`,
ADD COLUMN `production_status` VARCHAR(30) DEFAULT 'pending' COMMENT '生产状态：pending-待排程，scheduled-已排程，in_production-生产中，partial_completed-部分完成，completed-已完成' AFTER `remaining_qty`;

-- 更新已有数据的remaining_qty
UPDATE `sales_order_items` SET `remaining_qty` = `rolls` WHERE `remaining_qty` = 0 OR `remaining_qty` IS NULL;

-- 2. 修改订单主表状态
-- 订单状态扩展：pending/scheduled/printing/coating/rewinding/slitting/stripping/partial_completed/completed/cancelled
ALTER TABLE `sales_orders` 
MODIFY COLUMN `status` VARCHAR(30) NOT NULL DEFAULT 'pending' COMMENT '订单状态：pending-待处理，scheduled-已排程，printing-印刷中，coating-涂布中，rewinding-复卷中，slitting-分切中，stripping-分条中，partial_completed-部分完成，completed-已完成，cancelled-已取消';

-- 3. 库存表增加类型字段
ALTER TABLE `tape_stock` 
ADD COLUMN `stock_type` VARCHAR(20) DEFAULT 'finished' COMMENT '库存类型：jumbo-母卷，slit-支料，finished-成品' AFTER `product_name`,
ADD COLUMN `parent_batch_no` VARCHAR(50) COMMENT '来源批次号（分切/复卷后记录母卷批次）' AFTER `stock_type`,
ADD COLUMN `source_schedule_id` BIGINT COMMENT '来源排程ID' AFTER `parent_batch_no`;

-- 添加索引
CREATE INDEX `idx_stock_type` ON `tape_stock` (`stock_type`);
